...
public class ProductOp implements Polynomial {
    ...
    public ProductOp(Polynomial p1, Polynomial p2) {
        this.p1 = p1;
        this.p2 = p2;
    }
    @Override
    ...
    public Polynomial reduce() {
        ...
        if (reducedP2 instanceof Number) {
            if (reducedP2.evaluate(null) == 0) {
                return new Number(0);
            }
            if (reducedP2.evaluate(null) == 1) {
                return reducedP1.reduce();
            }
        }
        return new ProductOp(reducedP1, reducedP2);
    }

    @Override
    public boolean equals(Polynomial p) {
        if (!(p instanceof ProductOp)) {
            return false;
        }
